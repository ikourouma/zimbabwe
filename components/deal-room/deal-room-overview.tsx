...
export function DealRoomOverview() {
  const { isQualified, isAuthenticated, isLoading: authLoading } = useAuth();
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-white">Deal Room Overview</h1>
        <p className="text-sm mt-1 max-w-2xl" style={{ color: "var(--color-text-secondary)" }}>
          A private workspace for approved investors and government stakeholders to track deals through the
          governance workflow and log engagement on active projects.
        </p>
      </div>
