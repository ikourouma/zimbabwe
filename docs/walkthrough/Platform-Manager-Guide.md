...
One hundred and ninety-four records are held here, sorted most recent first, with category filters for projects (117), user and security (10), site settings (16), VDR and documents (1) and messages and hub (20). Each row gives the timestamp, the actor, the role they acted under, the action, the entity and its identifier, and a details column carrying the substance: a message created with its scope and its visibility to the investor recorded, five engagements created against named projects with the investor named on each, two taxonomy changes naming the term and whether a sector was added or removed, and an enquiry decision carrying its reason, its outcome and the applicant's address.

The details column is the part that carries assurance weight, because it preserves the substance of an act — which project, which investor, which term, which applicant — rather than only the fact that something happened. Note also whose entries these are: alongside the ZIDA Admin's enquiry decision sit the Platform Admin's own taxonomy changes, logged on identical terms. The tier that can alter the rules is recorded in the same ledger as the tier that operates within them, and the export puts that ledger in an auditor's hands without needing the platform to interpret it first.
...
The Government Executive Report is generated on demand and stamped: named to Amara Sesay as Platform Admin, timed to the second, referenced REP-20260907-3435, sourced to live platform data and marked for government use. Its command strip carries four measures — total pipeline value of US$2.34 billion across thirty-seven projects, investor capital coverage of 2.8 per cent representing US$61 million committed against US$2.21 billion published, average review turnaround of 10.9 days across thirty-one submissions, and a funnel conversion rate of 66.7 per cent. Below sit the counts — thirty-seven projects, twenty-six published, ten investor engagements, thirty-four registered users and seven leads received — and an economic impact block giving average project size of US$77.99 million across the thirty projects that carry a figure, and a publication rate of 70.3 per cent. A Pipeline by Status table then reconciles the portfolio stage by stage, from one draft through three submitted for review, four under review, one with changes requested and one approved to twenty-six published and one archived, and two charts close the document: capital required by sector, led by Manufacturing ahead of ICT and Infrastructure, and provincial pipeline distribution, led by Masvingo ahead of multi-province or national projects and Harare. Each chart carries a link to its underlying data table.
