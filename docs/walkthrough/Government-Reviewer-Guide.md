...
One inbox holds three threads, filtered as general enquiries, active deals and engagements. Threads on the left show message counts and the last contributor, titled by project where they concern one and by correspondent where they run on the general channel. The open thread concerns the TelOne Data Centres Project, with the project's own strip above it — published, USD29.97 million, Harare / Mashonaland Central / Bulawayo. Within the thread, a project amendment request is recorded as a card: the requesting ministry, the proposed changes to the capital requirement and the direct jobs figure, its routing for first review, and its standing as awaiting a decision from that ministry's administrator.
...
Four cards make up the vault. The first records the non-disclosure certificate as accepted on 6 September 2026 at 4:46:24 PM, against agreement version 1.0. The second, Company & accreditation, shows business registration not uploaded and offers slots for a commitment letter and a guarantee letter. The remaining two, MOU snapshots and recent document downloads, are empty on this account, reading no MOUs yet and no downloads recorded yet.
