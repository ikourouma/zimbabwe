...
The column on the right is the part a permanent secretary would want. Recent Activity names the actor and the exact transition — under review to approved, and approved back to under review on the same project — alongside postings to project threads and two engagements logged by Grace Mutindi, each named against the project it was raised on: the TelOne Fibre to the Home (FTTH) Deployment and the Kumusha Power Project. Several of those entries are ZIDA acts on this ministry's projects. So a ministry sees what central government has done to its portfolio, not only what its own officers did, and it sees which of its assets an investor approached rather than merely that an approach was made. That is the difference between being informed after the fact and holding the record.
...
Reports opens on the *Activity Report* — subtitled as investor engagements within your remit, and your account standing on the platform — and it is presented as a printable document rather than a dashboard. The header carries the Republic of Zimbabwe investment platform mark, a reference number, the generating officer named as Tapiwa Zvobgo (Ministry Admin), a snapshot timestamp given to the second, and live platform data declared as the source. A *Print / Save as PDF* control sits above it. The body states name, email, organisation and role, marks the Confidentiality Framework accepted, totals three engagements against US$30 million of investor-stated ticket value — counted across the two of the three in which the investor named a figure — with one pending review and two approved, and lists them under the heading *Engagements In Your Remit*, against projects under the ministry's portfolio, with the investor behind each approach, its status, indicative ticket, start date and next step. The investor column earns its place on this table above any other: the TelOne Fibre to the Home (FTTH) Deployment occupies two of the three rows, and it is only the naming of Lindiwe Ncube of Highveld Capital Partners against the submitted approach, carried at the project's own ask of US$50 million, and of Grace Mutindi of Zambezi Growth Partners against the approved one at US$25–100 million, that distinguishes two firms competing for the same asset from a single record entered twice; the remaining row is Grace Mutindi's approved approach on the Kumusha Power Project at US$5–25 million, which awaits completion of the MOU draft. Those three are the same three the Engagements console carries, so a report circulated from this page cannot disagree with the register it was drawn from.
