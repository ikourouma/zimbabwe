...
Four threads sit in the left-hand list — general questions on individual projects alongside an engagement thread on TelOne Fibre to the Home — filtered by all, general, active deals and engagements. The open thread shows an amendment request from a Qualified Investor proposing changes to the capital requirement and the direct jobs figure, rendered as a card marked approved, setting out each proposed value and naming Pilot ZIDA Admin as the officer who granted it. A note inside the card reads that the ministry is copied for transparency and that ZIDA Admin decides the final outcome. Composer selectors set visibility and audience.
